...
class PrePublishHook(Hook):
    ...
    def execute(self, tasks, work_template, progress_cb, **kwargs):
        ...
        for task in tasks:
            ...
            # pre-publish geo_cache output
            if output["name"] == "geo_cache":
                errors.extend(self._validate_item_for_geocache_publish(item))
            else:
    ...
    def _validate_item_for_geocache_publish(self, item):
        """
        Validate that the item is valid to be exported 
        to a geocache
        """
        errors = []
        # check that the cache set still exists:
        cacheSet = item["name"] + ':cache_SET'
        if not cmds.objExists(cacheSet):
            errors.append("This set couldn't be found in the scene!")
